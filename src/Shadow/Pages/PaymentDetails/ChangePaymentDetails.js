...
const changeDetails = async(e)=>{
    e.preventDefault()
    if(riderdata?.fullname.toLowerCase() !== formData.accountName.toLowerCase() ) {
        setErrMessage('Please Your Bank Account Holder Name Should Match Your Profile Name')
        setTimeout(() => {
            setErrMessage("")
            setFormData ({
                bankName: '',
    ...
    try{
        setLoading(true)
        const res = await fetch("https://ancient-wildwood-73926.herokuapp.com/delivery_agent_bank_account_change_request/change_bank_account", {
            method: "POST",
            headers: {
            ...
            body: JSON.stringify({
                ...
                new_account_type: formData.accountType,
      ...
      <section className="payment-details-section">
        <div className="payment-details-wrapper">
          ...
          <form className="payment-details-form" onSubmit={changeDetails}>
            ...
            <br />
            <input type="number" name="bvn" id="bvn" onChange={handleChange} value={formData.bvn}/>
            <br/>
